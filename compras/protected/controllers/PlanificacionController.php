...
/*			array('allow',  // allow all users to perform 'index' and 'view' actions
			...
			array('allow',  // allow all users to perform 'index' and 'view' actions
				'actions'=>array('index','view', 'vistaparcial', 'buscarespecfica', 'buscarespecficap', 
		 				'partidas', 'buscarpartida', 'buscargeneral', 'buscarNcm', 'buscarsubespecfica', 'buscarsubespecficap' ),
				'users'=>array('@'),
			...
			array('allow', // allow authenticated user to perform 'create' and 'update' actions
				'actions'=>array(  'crearUel', 'usuariosUel', 'misUELs', 'gesUsuUELs', 'rporusuario', 'ajaxreportes'),
				'users'=>array('@'),
				'roles'=>array('ente'),
				//'expression'=>'Yii::app()->controller->M_compras()',
			),
			...
			array('allow', // allow admin user to perform 'admin' and 'delete' actions
				'actions'=>array('crearente','misentes', 'usuariosentes', 'gesUsuEntes', 'actividad'),
				'users'=>array('@'),
				'roles'=>array('organo'),
				//'expression'=>'Yii::app()->controller->M_compras()',
			),
...
//PresupuestoPartidaAcciones::model()->findByAttributes(array('presupuesto_partida_id' => $id, 'ente_organo_id' => $usuario->ente_organo_id))
				...
				try{
		       	...
		       	if(!empty($presuPartida->presupuestoPartidaAcciones)){
					...
					foreach( PresupuestoPartidaAcciones::model()->findAllByAttributes(array('accion_id'=>$selId,'ente_organo_id'=>$usuario->ente_organo_id, 'anho' => Yii::app()->params['trimestresFechas'][Yii::app()->session['trimestreSeleccionado']]['anho'])) as $key => $value) {
					 	//Todos los presupuestos partidas de la Acción seleccionada
					 	$presupuestoPartidas[] = $value->presupuestoPartida;
					}
				}else{
